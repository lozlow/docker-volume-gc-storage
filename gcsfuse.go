...
func (d *gcpVolDriver) mountGcsfuse(volumeName string) error {
	...
	log.Printf("Running: $ gcsfuse --dir-mode 775 --file-mode 664 --key-file %s %s %s\n", d.gcpServiceKeyPath, bucketName, m)
	cmd := exec.Command("gcsfuse", "--dir-mode", "775", "--file-mode", "664", "--key-file", d.gcpServiceKeyPath, bucketName, m)
