...
func (d *gcpVolDriver) mountGcsfuse(volumeName string) error {
	...
	log.Printf("Running: $ gcsfuse --uid 33 --dir-mode 775 --file-mode 664 --key-file %s %s %s\n", d.gcpServiceKeyPath, bucketName, m)
	cmd := exec.Command("gcsfuse", "--uid", "33", "--dir-mode", "775", "--file-mode", "664", "--key-file", d.gcpServiceKeyPath, bucketName, m)
